...
# Se define antes que nada para evitar crasheos por falta de consola [cite: 1]
...
def solicitar_permisos_admin():
    ...
    if sys.argv[-1] != 'asadmin':
        ...
        ctypes.windll.shell32.ShellExecuteW(None, "runas", script, params, None, 1) [cite: 3]
        return True
    return False
...
def servicio_esta_instalado():
    try:
        res = subprocess.run('sc query "AgenteMonitoreo"', 
                             ...
                             creationflags=subprocess.CREATE_NO_WINDOW) [cite: 4]
        return "AgenteMonitoreo" in res.stdout
    except: return False

def instalar_servicio_automaticamente():
    exe_path = sys.executable
    subprocess.run('sc stop "AgenteMonitoreo"', shell=True, capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW) [cite: 4]
    time.sleep(1)
    subprocess.run('sc delete "AgenteMonitoreo"', shell=True, capture_output=True, creationflags=subprocess.CREATE_NO_WINDOW) [cite: 4]
    
    cmd = f'sc create "AgenteMonitoreo" binPath= "{exe_path}" start= auto DisplayName= "Agente de Monitoreo IT"' [cite: 5]
...
if RUNNING_AS_SERVICE:
    class AgenteMonitoreoService(win32serviceutil.ServiceFramework):
        ...
        _svc_description_ = "Sincronización de hardware con Firebase." [cite: 6]
        ...
        def SvcDoRun(self):
            ...
            try:
                ...
                datos = obtener_datos_pc() [cite: 7]
                enviar_datos_pc(datos) [cite: 9]
                escuchar_comandos_remotos(datos['uuid']) [cite: 10]
                
                while self.running:
                    rc = win32event.WaitForSingleObject(self.hWaitStop, 300000) [cite: 12]
                    if rc == win32event.WAIT_OBJECT_0:
                        break
                    enviar_datos_pc(obtener_datos_pc()) [cite: 12]
            except Exception as e:
                from src.database.firebase_client import log_debug
                log_debug(f"Error general en SvcDoRun: {e}") [cite: 13]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1].lower() in ['install', 'update', 'remove', 'start', 'stop']:
        if RUNNING_AS_SERVICE:
            win32serviceutil.HandleCommandLine(AgenteMonitoreoService) [cite: 13]
        sys.exit(0)
    ...
    if getattr(sys, 'frozen', False) and not sys.stdout:
        if RUNNING_AS_SERVICE:
            ...
            # CORRECCIÓN DEL ATRIBUTO AQUÍ:
            servicemanager.PrepareToHostSingle(AgenteMonitoreoService) [cite: 14]
            servicemanager.StartServiceCtrlDispatcher() [cite: 14]
        sys.exit(0)

    # Caso C: Usuario ejecuta el .exe con doble clic [cite: 15]
    if not servicio_esta_instalado():
        if not verificar_permisos_admin():
            solicitar_permisos_admin() [cite: 15]
        else:
            if instalar_servicio_automaticamente():
                print("Servicio instalado y corriendo.") [cite: 16]
                time.sleep(3)
    else:
